...
export async function buildMetadata(params: BuildMetadataParams = {}): Promise<Metadata> {
  ...
  if (isHomePage) {
    ...
    // 2. Site Ayarları -> Slogan
    const slogan = params.pageSeo?.metaTitle || siteTagline;
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://nuarkmimarlik.com";
  const canonicalUrl =
  ...
  const faviconUrl = defaults?.favicon?.asset?.url;
